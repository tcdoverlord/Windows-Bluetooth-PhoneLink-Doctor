...
function Set-DeviceSelection {
    ...
    while ($true) {
        ...
        Write-Host '  - To choose again later, use RESET from the main menu.'
        Write-Host ''
...
function Reset-DoctorSelections {
    Write-Section 'Reset Phone Link Bluetooth Doctor'
    Write-Host 'RESET clears the saved speaker, microphone, and Bluetooth phone choices.' -ForegroundColor Yellow
    Write-Host 'It does not remove devices, unpair Bluetooth, delete drivers, or change Windows audio.' -ForegroundColor Green
    $answer = Read-Host 'Type RESET exactly to return the Doctor to a clean first-run state'
    if ($answer -cne 'RESET') {
        Write-Host 'Reset cancelled. No changes were made.' -ForegroundColor Yellow
        return
    }

    Remove-Item -LiteralPath $script:SelectionFile -Force -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $script:PendingRestartFile -Force -ErrorAction SilentlyContinue

    Write-Host 'Reset complete.' -ForegroundColor Green
    Write-Host 'Choose RUN when ready. You will select the speaker, microphone, and phone again.' -ForegroundColor Cyan
}
...
function Show-RestartPrompt {
    if (-not $script:RestartRecommended) {
        Remove-Item -LiteralPath $script:PendingRestartFile -Force -ErrorAction SilentlyContinue
        Write-Host ''
        Write-Host '======================================================================' -ForegroundColor Green
        Write-Host ' Repair completed. No computer restart is required.' -ForegroundColor Green
        Write-Host ' Test your microphone, speakers, and Phone Link now.' -ForegroundColor Cyan
        Write-Host ' If Windows audio still behaves incorrectly, restart only as a final test.' -ForegroundColor Yellow
        Write-Host '======================================================================' -ForegroundColor Green
        return
    }

    Save-PendingRestart -Reason 'A Windows Bluetooth audio service could not finish restarting normally.'

    Clear-Host
    Write-Host ''
    Write-Host '######################################################################' -ForegroundColor Yellow
    Write-Host '#                                                                    #' -ForegroundColor Yellow
    Write-Host '#                 COMPUTER RESTART RECOMMENDED                       #' -ForegroundColor Red
    Write-Host '#                                                                    #' -ForegroundColor Yellow
    Write-Host '######################################################################' -ForegroundColor Yellow
    Write-Host ''
    Write-Host 'The Phone Link Bluetooth Doctor repair is complete.' -ForegroundColor Green
    Write-Host 'A Bluetooth audio service could not finish changing normally.' -ForegroundColor Yellow
    Write-Host 'Save your work and restart Windows before testing Phone Link again.' -ForegroundColor White
    Write-Host ''
...
function Show-Menu {
    while ($true) {
        ...
        Write-Host '1. RUN - diagnose and safely repair Phone Link Bluetooth audio'
        Write-Host '2. RESET - clear saved device choices and start clean'
        Write-Host 'X. EXIT'
        $choice = Read-Host 'Choose 1, 2, or X'
        ...
        switch -Regex ($choice) {
            ...
            '^2$' { Reset-DoctorSelections }
            '^(?i)x$' { return }
            default { Write-Host 'Invalid choice. Choose 1, 2, or X.' -ForegroundColor Red }
        }
    }
}
